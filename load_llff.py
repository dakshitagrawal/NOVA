...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    dispfiles = [
        os.path.join(disp_dir, f)
        for f in sorted(os.listdir(disp_dir))
        if f.endswith("npy")
    ]

    disp = [
        cv2.resize(np.load(f), (sh[1], sh[0]), interpolation=cv2.INTER_NEAREST)
        for f in dispfiles
    ]
    ...
    for i in range(num_img):
        if i == num_img - 1:
            ...
            fwd_mask = cv2.resize(
                fwd_mask, (sh[1], sh[0]), interpolation=cv2.INTER_NEAREST
            )
        ...
        if i == 0:
            ...
            bwd_mask = cv2.resize(
                bwd_mask, (sh[1], sh[0]), interpolation=cv2.INTER_NEAREST
            )
...
def load_llff_data(
    ...
    )  # factor=2 downsamples original imgs by 2x
    ...
    sc = 1.0 if bd_factor is None else 1.0 / (np.percentile(bds[:, 0], 5) * bd_factor)
    ...
    grids = get_grid(
        int(H), int(W), len(poses), flows_f, flow_masks_f, flows_b, flow_masks_b
    )  # [N, H, W, 8]

    return images, disp, masks, poses, bds, render_poses, render_focals, grids
...
def generate_path(c2w, args):
    ...
    for i in range(num_novelviews):
        x_trans = (
            max_trans
            * np.sin(2.0 * np.pi * float(i) / float(num_novelviews))
            * args.x_trans_multiplier
        )
        y_trans = (
            max_trans
            * (np.cos(2.0 * np.pi * float(i) / float(num_novelviews)) - 1.0)
            * args.y_trans_multiplier
        )
        z_trans = 0.0
    ...
    if args.frame2dolly != -1:
        for i in range(num_novelviews // 2 + 1):
            ...
            z_trans = (
                max_trans * args.z_trans_multiplier * i / float(num_novelviews // 2)
            )
            i_pose = np.concatenate(
                [
                    np.concatenate(
    ...
    if args.frame2dolly != -1:
        for i in range(num_novelviews // 2 + 1):
            ...
            new_focal = (
                focal
                - args.focal_decrease * z_trans / max_trans / args.z_trans_multiplier
            )
